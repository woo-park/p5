...
//question--// (1.0 points) The ball should be assigned a random X and Y speed when the game starts
...
//question -- how accurate does it has to be // (0.5 points) If the ball hits the paddle it should "bounce" and reverse its Y speed
...
let text_font_size = 180;
let counter = 0;
let textX = w/2;
let textY = h/2;

let miss_font_size = 50;
let miss_counter = 0;
let miss_textX = w - 60;
let miss_textY = 60;
...
function draw(){
    ...
    text(miss_counter,miss_textX, miss_textY)
    pop()

    push()
    fill(0, 102, 153, 120);
    text(counter, textX, textY)
    pop()
    
    //restrict movement
    paddleX = constrain(paddleX, 0 + right_wall, w - paddleWidth - left_wall)   

    ball_speedX = random_speedX;    //hm
    ball_speedY = random_speedY;

    posX += ball_speedX * dirX;
    posY += ball_speedY * dirY;

    ellipse(posX,posY,10,10)

    if(posX >= (width - right_wall) || posX <= 0 + left_wall) {         // right wall & left wall
        dirX *= -1;
    }
    if( posY <= 0 + top_wall) {          //   top wall
        dirY *= -1;
    }

     // ball collision with paddle - then reverse
    if (dist(posX, posY, paddleX + paddleWidth/2 , paddleY) < paddleHeight) {
        dirY *= -1;
    }
    ...
    if(dist(posX, posY, objectX, objectY) < 25 ) {
        objectX = random(45, width-45);
        objectY = random(45, height-45);
        counter += 1;
    }
...
function mousePressed(){
    random_speedX = random(-4,4);
    random_speedY = random(-4,4);
